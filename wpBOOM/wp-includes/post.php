...
function wp_delete_post( $postid = 0, $force_delete = false ) {
	...
	if ( !$force_delete && ( $post->post_type == 'post' || $post->post_type == 'page') && get_post_status( $postid ) != 'trash' && EMPTY_TRADAYS )
...
function wp_delete_attachment( $post_id, $force_delete = false ) {
	...
	if ( !$force_delete && EMPTY_TRADAYS && MEDIA_TRASH && 'trash' != $post->post_status )
...
function wp_mime_type_icon( $mime = 0 ) {
	...
	if ( empty($icon) ) {
		...
		if ( !is_array($icon_files) ) {
			...
			$icon_dir_PATH = apply_filters( 'icon_dir_PATH', includes_url('images/crystal') );
			$dirs = apply_filters( 'icon_dirs', array($icon_dir => $icon_dir_PATH) );
			$icon_files = array();
			while ( $dirs ) {
