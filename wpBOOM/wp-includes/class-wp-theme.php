...
final class WP_Theme implements ArrayAccess {
	...
	public function __isset( $offset ) {
		static $properties = array(
			...
			'screenshot', 'description', 'author', 'tags', 'theme_root', 'theme_root_PATH',
		);
	...
	public function __get( $offset ) {
		switch ( $offset ) {
			...
			case 'theme_root_PATH' :
				return $this->get_theme_root_PATH();
	...
	public function cache_delete() {
		...
		$this->template = $this->textdomain_loaded = $this->theme_root_PATH = $this->parent = $this->errors = $this->headers_sanitized = $this->name_translated = null;
		$this->headers = array();
	 ...
	 */
	public function get_stylesheet_directory_PATH() {
		return $this->get_theme_root_PATH() . '/' . str_replace( '%2F', '/', rawurlencode( $this->stylesheet ) );
	}
	 ...
	 */
	public function get_template_directory_PATH() {
		if ( $this->parent() )
			$theme_root_PATH = $this->parent()->get_theme_root_PATH();
		else
			$theme_root_PATH = $this->get_theme_root_PATH();

		return $theme_root_PATH . '/' . str_replace( '%2F', '/', rawurlencode( $this->template ) );
	}
	 ...
	 * get_theme_root_PATH() and allow it to run the theme_root_PATH filter.
	 *
	 * @uses get_theme_root_PATH()
	 *
	 * @since 3.4.0
	 * @access public
	 *
	 ...
	 */
	public function get_theme_root_PATH() {
		if ( ! isset( $this->theme_root_PATH ) )
			$this->theme_root_PATH = get_theme_root_PATH( $this->stylesheet, $this->theme_root );
	...
	public function get_screenshot( $uri = 'uri' ) {
		...
		if ( $screenshot ) {
			if ( 'relative' == $uri )
				return $screenshot;
			return $this->get_stylesheet_directory_PATH() . '/' . $screenshot;
		} elseif ( 0 === $screenshot ) {
			return false;
		}
		...
		foreach ( array( 'png', 'gif', 'jpg', 'jpeg' ) as $ext ) {
			if ( file_exists( $this->get_stylesheet_directory() . "/screenshot.$ext" ) ) {
				...
				return $this->get_stylesheet_directory_PATH() . '/' . 'screenshot.' . $ext;
			}
		}
