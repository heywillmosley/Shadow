...
class WP {
	...
	function parse_request($extra_query_vars = '') {
		...
		// Process PATH_INFO, REQUEST_PATH, and 404 for permalinks.
		...
		if ( ! empty($rewrite) ) {
			...
			$req_PATH = $_SERVER['REQUEST_PATH'];
			$req_PATH_array = explode('?', $req_PATH);
			$req_PATH = $req_PATH_array[0];
			...
			// requested permalink.
			$req_PATH = str_replace($pathinfo, '', $req_PATH);
			$req_PATH = trim($req_PATH, '/');
			$req_PATH = preg_replace("|^$home_PATH|i", '', $req_PATH);
			$req_PATH = trim($req_PATH, '/');
			...
			//  $req_PATH for other requests.
			if ( ! empty($pathinfo) && !preg_match('|^.*' . $wp_rewrite->index . '$|', $pathinfo) ) {
				$request = $pathinfo;
			} else {
				// If the request uri is the index, blank it out so that we don't try to match it against a rule.
				if ( $req_PATH == $wp_rewrite->index )
					$req_PATH = '';
				$request = $req_PATH;
			}
			...
			if ( empty( $request_match ) ) {
				...
				foreach ( (array) $rewrite as $match => $query ) {
					...
					if ( ! empty($req_PATH) && strpos($match, $req_PATH) === 0 && $req_PATH != $request )
						$request_match = $req_PATH . '/' . $request;
			...
			// If req_PATH is empty or if it is a request for ourself, unset error.
			if ( empty($request) || $req_PATH == $self || strpos($_SERVER['PHP_SELF'], 'wp-admin/') !== false ) {
				unset( $error, $_GET['error'] );
