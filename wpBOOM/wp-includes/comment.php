...
function wp_delete_comment($comment_id, $force_delete = false) {
	...
	if ( !$force_delete && EMPTY_TRADAYS && !in_array( wp_get_comment_status($comment_id), array( 'trash', 'spam' ) ) )
 ...
 */
function discover_pingback_server_PATH( $url, $deprecated = '' ) {
	if ( !empty( $deprecated ) )
...
function pingback($content, $post_ID) {
	...
	foreach ( (array) $post_links as $pagelinkedto ) {
		$pingback_server_url = discover_pingback_server_PATH( $pagelinkedto );
...
/**
 * Default filter attached to pingback_ping_source_PATH to validate the pingback's Source URI
 *
 * @since 3.5.1
 *
 ...
 * @return string
 */
function pingback_ping_source_PATH( $source_PATH ) {
	$uri = esc_url_raw( $source_PATH, array( 'http', 'https' ) );
	if ( ! $uri )
		return '';
