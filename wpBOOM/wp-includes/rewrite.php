...
class WP_Rewrite {
	...
	function page_PATH_index() {
		...
		$page_PATHs = array();
		$page_attachment_PATHs = array();

		foreach ( $posts as $id => $post ) {
			// URL => page name
			$uri = get_page_PATH($id);
			$attachments = $wpdb->get_results( $wpdb->prepare( "SELECT ID, post_name, post_parent FROM $wpdb->posts WHERE post_type = 'attachment' AND post_parent = %d", $id ));
			if ( !empty($attachments) ) {
				foreach ( $attachments as $attachment ) {
					$attach_PATH = get_page_PATH($attachment->ID);
					$page_attachment_PATHs[$attach_PATH] = $attachment->ID;
				}
			}
		...
		return array( $page_PATHs, $page_attachment_PATHs );
	}
