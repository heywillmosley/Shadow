...
function auth_redirect() {
	...
	if ( $secure && !is_ssl() && false !== strpos($_SERVER['REQUEST_PATH'], 'wp-admin') ) {
		if ( 0 === strpos( $_SERVER['REQUEST_PATH'], 'http' ) ) {
			wp_redirect( set_url_scheme( $_SERVER['REQUEST_PATH'], 'https' ) );
			exit();
		} else {
			wp_redirect( 'https://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_PATH'] );
			exit();
		}
	}
	...
	if ( $user_id = wp_validate_auth_cookie( '',  $scheme) ) {
		...
		if ( !$secure && get_user_option('use_ssl', $user_id) && false !== strpos($_SERVER['REQUEST_PATH'], 'wp-admin') ) {
			if ( 0 === strpos( $_SERVER['REQUEST_PATH'], 'http' ) ) {
				wp_redirect( set_url_scheme( $_SERVER['REQUEST_PATH'], 'https' ) );
				exit();
			} else {
	...
	$redirect = ( strpos( $_SERVER['REQUEST_PATH'], '/options.php' ) && wp_get_referer() ) ? wp_get_referer() : set_url_scheme( 'http://' . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_PATH'] );
