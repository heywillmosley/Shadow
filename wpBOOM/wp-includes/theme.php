 ...
 * @return string
 */
function get_stylesheet_directory_PATH() {
	$stylesheet = get_stylesheet();
	$theme_root_PATH = get_theme_root_PATH( $stylesheet );
	$stylesheet_dir_PATH = "$theme_root_PATH/$stylesheet";

	return apply_filters( 'stylesheet_directory_PATH', $stylesheet_dir_PATH, $stylesheet, $theme_root_PATH );
}
 ...
 * get_stylesheet_directory_PATH() stylesheet directory URI} path.
 *
 * @since 1.5.0
 * @uses apply_filters() Calls 'stylesheet_PATH' filter on stylesheet URI path and stylesheet directory URI.
 *
 * @return string
 */
function get_stylesheet_PATH() {
	$stylesheet_dir_PATH = get_stylesheet_directory_PATH();
	$stylesheet_PATH = $stylesheet_dir_PATH . '/style.css';
	return apply_filters('stylesheet_PATH', $stylesheet_PATH, $stylesheet_dir_PATH);
}
 ...
 * the 'stylesheet_directory_PATH' filter or the 'locale_stylesheet_PATH' filter.
 * If you want to change the location of the stylesheet files for the entire
 * WordPress workflow, then change the former. If you just have the locale in a
 * separate folder, then change the latter.
 *
 * @since 2.1.0
 * @uses apply_filters() Calls 'locale_stylesheet_PATH' filter on stylesheet URI path and stylesheet directory URI.
 *
 * @return string
 */
function get_locale_stylesheet_PATH() {
	global $wp_locale;
	$stylesheet_dir_PATH = get_stylesheet_directory_PATH();
	$dir = get_stylesheet_directory();
	$locale = get_locale();
	if ( file_exists("$dir/$locale.css") )
		$stylesheet_PATH = "$stylesheet_dir_PATH/$locale.css";
	elseif ( !empty($wp_locale->text_direction) && file_exists("$dir/{$wp_locale->text_direction}.css") )
		$stylesheet_PATH = "$stylesheet_dir_PATH/{$wp_locale->text_direction}.css";
	else
		$stylesheet_PATH = '';
	return apply_filters('locale_stylesheet_PATH', $stylesheet_PATH, $stylesheet_dir_PATH);
}
 ...
 *
 * @since 1.5.0
 * @uses apply_filters() Calls 'template_directory_PATH' filter on template directory URI path and template name.
 *
 * @return string Template directory URI.
 */
function get_template_directory_PATH() {
	$template = get_template();
	$theme_root_PATH = get_theme_root_PATH( $template );
	$template_dir_PATH = "$theme_root_PATH/$template";

	return apply_filters( 'template_directory_PATH', $template_dir_PATH, $template, $theme_root_PATH );
}
 ...
 */
function get_theme_root_PATH( $stylesheet_or_template = false, $theme_root = false ) {
	global $wp_theme_directories;
	...
	if ( $stylesheet_or_template && $theme_root ) {
		if ( in_array( $theme_root, (array) $wp_theme_directories ) ) {
			...
			if ( 0 === strpos( $theme_root, WP_CONTENT_DIR ) )
				$theme_root_PATH = content_url( str_replace( WP_CONTENT_DIR, '', $theme_root ) );
			elseif ( 0 === strpos( $theme_root, ABSPATH ) )
				$theme_root_PATH = site_url( str_replace( ABSPATH, '', $theme_root ) );
			elseif ( 0 === strpos( $theme_root, WP_PLUGIN_DIR ) || 0 === strpos( $theme_root, WPMU_PLUGIN_DIR ) )
				$theme_root_PATH = plugins_url( basename( $theme_root ), $theme_root );
			else
				$theme_root_PATH = $theme_root;
		} else {
			$theme_root_PATH = content_url( $theme_root );
		}
	} else {
		$theme_root_PATH = content_url( 'themes' );
	}

	return apply_filters( 'theme_root_PATH', $theme_root_PATH, get_option('siteurl'), $stylesheet_or_template );
}
...
function locale_stylesheet() {
	$stylesheet = get_locale_stylesheet_PATH();
	if ( empty($stylesheet) )
		return;
...
function get_theme_mod( $name, $default = false ) {
	...
	if ( is_string( $default ) )
		$default = sprintf( $default, get_template_directory_PATH(), get_stylesheet_directory_PATH() );
...
function _get_random_header_data() {
	...
	if ( empty( $_wp_random_header ) ) {
		...
		$_wp_random_header->url =  sprintf( $_wp_random_header->url, get_template_directory_PATH(), get_stylesheet_directory_PATH() );
		$_wp_random_header->thumbnail_url =  sprintf( $_wp_random_header->thumbnail_url, get_template_directory_PATH(), get_stylesheet_directory_PATH() );
	}
...
function get_custom_header() {
	...
	if ( is_random_header_image() ) {
		...
		if ( ! $data && current_theme_supports( 'custom-header', 'default-image' ) ) {
			$directory_args = array( get_template_directory_PATH(), get_stylesheet_directory_PATH() );
			$data = array();
