 ...
 * an empty string removes the key. Omitting oldquery_or_PATH uses the $_SERVER
...
function wp_referer_field( $echo = true ) {
	$ref = esc_attr( $_SERVER['REQUEST_PATH'] );
...
function wp_original_referer_field( $echo = true, $jump_back_to = 'current' ) {
	$jump_back_to = ( 'previous' == $jump_back_to ) ? wp_get_referer() : $_SERVER['REQUEST_PATH'];
...
function wp_get_referer() {
	...
	if ( $ref && $ref !== $_SERVER['REQUEST_PATH'] )
		return $ref;
	return false;
}
...
function wp_guess_url() {
	if ( defined('WP_SITEURL') && '' != WP_SITEURL ) {
		...
		$url = preg_replace( '#/(wp-admin/.*|wp-login.php)#i', '', $schema . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_PATH'] );
	}
...
/**
 * Permanently deletes posts, pages, attachments, and comments which have been in the trash for EMPTY_TRADAYS.
 *
 * @since 2.9.0
 */
function wp_scheduled_delete() {
	...
	$delete_timestamp = time() - ( DAY_IN_SECONDS * EMPTY_TRADAYS );
